...
**Very simple README** — quick steps to set up and run the RAG demo.
...
```bash
uv run --refresh --with git+https://github.com/SushantGautam/vllm-RAG.git -- ingest_documents
```

3) Start the server:

```bash
uv run --refresh --with git+https://github.com/SushantGautam/vllm-RAG.git -- start-server
```
